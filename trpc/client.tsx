 ...
 * ```tsx
 * // Query
 * const { data } = trpc.organization.startup.list.useQuery({ limit: 10 });
 *
 * // Mutation
 * const mutation = trpc.organization.startup.create.useMutation();
 * await mutation.mutateAsync({ name: "Backer", founderName: "Jane", email: "j@backer.com" });
 * ```
 *
...
/**
 * Internal component that syncs organization scope with QueryClient.
			...
			trpc.createClient({
				links: [
					...
					httpBatchLink({
						...
						/**
						 * Relative URL resolves to the **current page origin** on each request.
						 * Using `getBaseUrl()` can send API calls to localhost:3000 while the user
						 * opens `http://localhost`, another port, or 127.0.0.1 — cookies miss match
						 * and `organization.list` can hang or never authorize.
						 */
						url: "/api/trpc",
					}),
				],
